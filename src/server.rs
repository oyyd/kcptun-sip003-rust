...
const MAX_FRAME_DATA_LEN: usize = 65535;
...
impl Server {
  ...
  pub async fn run(&self) -> Result<()> {
    ...
    loop {
      ...
      if self.config.server_kcp_stream_rw_timeout.is_some() {
        kcp_stream_with_timeout.set_read_timeout(self.config.server_kcp_stream_rw_timeout);
        kcp_stream_with_timeout.set_write_timeout(self.config.server_kcp_stream_rw_timeout);
      }
